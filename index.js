...
async function startBot() {
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    // 🔐 CODE SECRET LAVAWULE (pour déverrouiller les commandes)
  ...
  // 🔐 DEMANDE DU PAIRING CODE PERSONNALISÉ = LAVAWULE
  if (!pairingCodeRequested) {
    pairingCodeRequested = true;
    const phoneNumber = '242044106402';  // Ton numéro
    const customCode = 'LAVAWULE';        // Code personnalisé
    
    console.log(`📞 Demande du code personnalisé "${customCode}" pour le numéro : ${phoneNumber}`);
    
    try {
      const code = await sock.requestPairingCode(phoneNumber, customCode);
      console.log(`\n✅ CODE D'ASSOCIATION ENVOYÉ : ${code}`);
      console.log(`📱 Sur WhatsApp : Paramètres → Appareils liés → Lier avec un numéro\n`);
    } catch (err) {
