...
async function startBot() {
  ...
  sock = makeWASocket({
    ...
    browser: ['LA VAWULENCE', 'Chrome', '1.0.0']   // ← Pour pairing code
  });

  sock.ev.on('connection.update', async (update) => {
    const { connection, lastDisconnect, pairingCode } = update;
    
    // 🔐 AFFICHER LE PAIRING CODE (NOUVEAU)
    if (pairingCode) {
    ...
    if (connection === 'open') {
      ...
      // Photo de profil du bot (déjà existant)
      try {
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    // 🔐 CODE SECRET LAVAWULE (déjà existant)
    ...
    // Device fingerprint (déjà existant)
    ...
    // Exécuter la commande (déjà existant)
  ...
  // 🔐 DEMANDER LE PAIRING CODE POUR TON NUMÉRO (NOUVEAU)
